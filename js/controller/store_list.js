APP.controller.storeList = (function() {
	var api = {},thisView,cmCtrl, table,rowData,rowIndex,sts;
	api.start = function () {
		cmCtrl = APP.controller.common;
		APP.view.store_list.loadBody(function() {
			addEvent();
	...
	function addEvent() {
		var dt = 'click';
		$('#add').on(dt,function() {
			ichain.invokeParent('APP.controller.admin.childCallChangeUrl',{"page":'store_add.html'});
		});
		//点击修改提交
		...
		//查询数据
		$(document).on(dt,'#search',function(e) {
			e.preventDefault();
			var data = {};
			$.each($('.searchform').serializeArray(),function(index,node) {
				data[node.name] = node.value;
			});
			table.bootstrapTable($(this).data('method'), getRows(data));
			$('.search').focus();
	...
	function getRows(data) {
		return rps = cmCtrl.doAjax(data,'../data/table.json');
	}
	...
	function initTable(dom,url) {
		table = $('#'+dom).bootstrapTable({
            ...
            cache: false,
//          height: 400,
            responseHandler:function(ret) {return ret.rows},
            striped: true,
            pagination: true,
            pageSize: 10,
