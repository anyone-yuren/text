APP.controller.subaccountList = (function() {
	var api = {};
	api.start = function () {
		APP.view.subaccountList.loadBody(function() {
			
			addEvent();
			initTable('sublist','../data/table2.json');
		});
	}
	function addEvent() {
		var dt = 'click';
		$('#add').on(dt,function() {
			ichain.invokeParent('APP.controller.admin.childCallChangeUrl',{"page":'subaccount_add.html'});
		});
		//点击修改提交
	...
	function initTable(dom,url) {
		table = $('#'+dom).bootstrapTable({
            ...
            cache: false,
//          height: 400,
            responseHandler:function(ret) {return ret},
            striped: true,
            pagination: true,
            pageSize: 10,
